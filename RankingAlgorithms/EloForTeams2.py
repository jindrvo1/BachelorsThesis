"""
In this approach, I change the x, y and K parameters while rating players.
Training is done using the cross-entropy method.

~541/1520 correctly classified.
"""

from math import exp, sqrt, log
from random import seed, random, normalvariate

class EloForTeams2(object):
    ...
    def train(self, tr):
        ...
        for i in range(10):
            ...
            temperature = 10000
            cooling_rate = 0.03
            best = (self.log_likelihood(tr, x, y), x, y)
            explore = best
            seed()

            while temperature > 1:
                x_new, y_new = self.mutate(x, y)
                fitness_new = self.log_likelihood(tr, x_new, y_new)
        ...
        print("Log-likelihood: {}".format(self.log_likelihood(tr, total_best[1], total_best[2])))
        print("Predicted matches: {}".format(self.fitness(tr, total_best[1], total_best[2])))
    ...
    def log_likelihood(self, tr, x, y):
        e_f = lambda r_t1, r_t2, x, y: 1.0/(1+x**((r_t2 - r_t1)/y))
        log_likelihood = 0
